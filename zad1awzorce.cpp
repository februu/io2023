...
class MyComputerAssembler {
public:
 Computer assembleComputer(ComputerBuilder& builder) {
 builder.buildCPU("AMD Ryzen 5 3600");
...
int main() {
 ...
 desktop.display();
 MyComputerAssembler myAssembler;
 desktop = myAssembler.assembleComputer(desktopBuilder);
 std::cout << "My Desktop Computer Configuration:" << std::endl;
 desktop.display();
 return 0;
}
